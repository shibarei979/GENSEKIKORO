...
export default function MissionClient({ user, stats, initialClaimedIds }: Props) {
  ...
  const categoryColors: Record<string, {bg:string;border:string;color:string;bar:string}> = {
    '読者':    {bg:'var(--color-brand-light)',border:'var(--color-tag-border)',color:'var(--color-brand)',bar:'var(--color-brand)'},
      ...
      <div style={{
        ...
        <div style={{display:'flex',alignItems:'flex-start',justifyContent:'space-between',gap:12}}>
          <div style={{flex:1,minWidth:0}}>
            <div style={{fontSize:14,fontWeight:700,color:'var(--color-text)',marginBottom:3}}>{m.title}</div>
            <div style={{fontSize:12,color:'var(--color-text-muted)',marginBottom:8}}>{m.description}</div>
            <div style={{display:'flex',alignItems:'center',gap:10}}>
              <div style={{flex:1,height:5,background:'var(--color-brand-border)',borderRadius:3,overflow:'hidden'}}>
                <div style={{height:'100%',background:done?color.bar:'var(--color-brand)',borderRadius:3,width:`${pct}%`,transition:'width .3s'}}/>
              </div>
              <span style={{fontSize:11,color:done?color.color:'var(--color-text-muted)',fontWeight:600,whiteSpace:'nowrap'}}>
                {user ? `${m.current} / ${m.target}` : `? / ${m.target}`}
              </span>
              {!done && <span style={{fontSize:11,color:'var(--color-text-faint)',whiteSpace:'nowrap'}}>あと{m.target - m.current}</span>}
            </div>
          </div>
          {done && (
            <button onClick={()=>setClaimTarget({id:m.id,badgeName:m.badgeName,title:m.title,claimIds:[m.id]})}
              style={{padding:'7px 16px',background:'var(--color-brand)',color:'var(--color-text-inverse)',border:'none',borderRadius:20,fontSize:13,fontWeight:700,cursor:'pointer',whiteSpace:'nowrap',boxShadow:'0 2px 8px color-mix(in srgb, var(--color-brand) 30%, transparent)',flexShrink:0}}>
              クリア！
            </button>
          )}
        </div>
  ...
  return (
    <div>
      ...
      <div style={{marginBottom:20}}>
        <h1 style={{fontSize:22,fontWeight:700,color:'var(--color-text)',margin:0,marginBottom:6,fontFamily:"'Noto Serif JP',serif"}}>ミッション</h1>
        <p style={{fontSize:13,color:'var(--color-text-muted)',margin:0,lineHeight:1.7}}>ミッションをクリアしてバッジを獲得しよう。</p>
      </div>

      {/* 全体進捗 */}
      <div style={{background:'var(--color-bg-card)',border:'1px solid var(--color-brand-border)',borderRadius:14,padding:'16px 20px',marginBottom:20}}>
        <div style={{display:'flex',alignItems:'center',justifyContent:'space-between',marginBottom:10}}>
          <span style={{fontSize:14,fontWeight:700,color:'var(--color-text)'}}>全体の達成状況</span>
          <span style={{fontSize:13,color:'var(--color-brand)',fontWeight:700}}>{completedMissions} / {totalMissions}</span>
        </div>
        <div style={{height:8,background:'var(--color-brand-border)',borderRadius:4,overflow:'hidden'}}>
          <div style={{height:'100%',background:'var(--color-brand)',borderRadius:4,width:`${(completedMissions/totalMissions)*100}%`,transition:'width .4s'}}/>
        </div>
        {!user && (
          <div style={{marginTop:12,fontSize:12,color:'var(--color-text-muted)',background:'var(--color-bg)',border:'1px solid var(--color-brand-border)',borderRadius:8,padding:'8px 12px'}}>
            <a href="/auth/login" style={{color:'var(--color-brand)',fontWeight:600}}>ログイン</a>すると進捗が保存されます
          </div>
        )}
      </div>
        ...
        return (
          <div key={quest.id} style={{marginBottom:20}}>
            <div style={{display:'flex',alignItems:'center',gap:10,marginBottom:8}}>
              ...
              <span style={{fontSize:15,fontWeight:700,color:'var(--color-text)'}}>期間クエスト</span>
              ...
              {quest.deadline && (
                <span style={{fontSize:11,color:'var(--color-text-faint)',marginLeft:'auto'}}>〜{quest.deadline}</span>
              )}
            </div>

            {/* クエストまとめカード（押すと展開） */}
            <div style={{background:'var(--color-bg-card)',border:'1px solid var(--color-brand-border)',borderRadius:12,overflow:'hidden',marginBottom: questOpen ? 0 : 0}}>
              {/* ヘッダー：クリックで展開 */}
              <button
              ...
              >
                <div style={{flex:1,minWidth:0}}>
                  <div style={{fontSize:14,fontWeight:700,color:'var(--color-text)',marginBottom:3}}>{quest.title}</div>
                  <div style={{fontSize:12,color:'var(--color-text-muted)'}}>期間クエストをクリアしよう</div>
                </div>
              ...
              {/* 展開：クエスト内のミッション一覧 */}
              {questOpen && (
                <div style={{borderTop:'1px solid var(--color-brand-border)',padding:'12px 20px 16px'}}>
                  {quest.missions.map(m => {
                    ...
                    return (
                      <div key={m.id} style={{
                        ...
                        <div style={{display:'flex',alignItems:'center',gap:10}}>
                          <div style={{flex:1,height:5,background:'var(--color-brand-border)',borderRadius:3,overflow:'hidden'}}>
                            <div style={{height:'100%',background:done?'#e11d48':'var(--color-brand)',borderRadius:3,width:`${pct}%`,transition:'width .3s'}}/>
                          </div>
                          <span style={{fontSize:11,color:done?'#e11d48':'var(--color-text-muted)',fontWeight:600,whiteSpace:'nowrap'}}>
                            {user ? `${m.current} / ${m.target}` : `? / ${m.target}`}
                          </span>
                          {!done && <span style={{fontSize:11,color:'var(--color-text-faint)',whiteSpace:'nowrap'}}>あと{m.target - m.current}</span>}
                        </div>
                  ...
                  {questDone ? (
                    <button
                      ...
                      style={{width:'100%',padding:'12px',background:'#e11d48',color:'var(--color-text-inverse)',border:'none',borderRadius:10,fontSize:14,fontWeight:700,cursor:'pointer',marginTop:4,boxShadow:'0 2px 8px rgba(225,29,72,.3)'}}>
                      クリア！バッジを受け取る
                    </button>
                  ) : (
                    <div style={{textAlign:'center',fontSize:12,color:'var(--color-text-faint)',marginTop:4}}>
        ...
        return (
          <div key={cat} style={{marginBottom:16}}>
            <div style={{display:'flex',alignItems:'center',gap:10,marginBottom:8}}>
              ...
              <span style={{fontSize:15,fontWeight:700,color:'var(--color-text)'}}>{cat}ミッション</span>
      ...
      {visibleMissions.length === 0 && seasonalQuests.every(q=>claimedIds.has(q.id)) && (
        <div style={{background:'var(--color-brand-light)',border:'1px solid var(--color-tag-border)',borderRadius:14,padding:'40px',textAlign:'center',marginBottom:16}}>
          <div style={{fontSize:16,fontWeight:700,color:'var(--color-brand)',marginBottom:6}}>全ミッション達成！</div>
          <div style={{fontSize:13,color:'var(--color-text-muted)'}}>新しいミッションが追加されるまでお待ちください</div>
        </div>
      )}

      <div style={{background:'var(--color-bg)',border:'1px solid var(--color-brand-border)',borderRadius:14,padding:'16px 20px',marginTop:8}}>
        <div style={{fontSize:13,fontWeight:700,color:'var(--color-text)',marginBottom:6}}>近日公開予定</div>
      ...
      {claimTarget && (
        <div style={{position:'fixed',inset:0,zIndex:1000,display:'flex',alignItems:'center',justifyContent:'center',padding:24}}>
          ...
          <div style={{position:'relative',background:'var(--color-bg-card)',borderRadius:20,padding:'32px 28px',maxWidth:320,width:'100%',textAlign:'center',boxShadow:'0 20px 60px rgba(0,0,0,0.2)',zIndex:1}}>
            <div style={{fontSize:11,color:'var(--color-brand)',fontWeight:700,letterSpacing:'0.15em',marginBottom:16}}>バッジ獲得</div>
            <div style={{width:100,height:100,borderRadius:'50%',background:'linear-gradient(135deg,var(--color-brand),#f5a060)',margin:'0 auto 20px',display:'flex',alignItems:'center',justifyContent:'center',boxShadow:'0 8px 24px color-mix(in srgb, var(--color-brand) 35%, transparent)',border:'4px solid var(--color-brand-light)'}}>
              <span style={{fontSize:13,fontWeight:700,color:'var(--color-text-inverse)',lineHeight:1.4,padding:'0 8px'}}>仮バッジ</span>
            </div>
            <div style={{fontSize:18,fontWeight:700,color:'var(--color-text)',marginBottom:6,fontFamily:"'Noto Serif JP',serif"}}>{claimTarget.badgeName}</div>
            <div style={{fontSize:13,color:'var(--color-text-muted)',marginBottom:6}}>「{claimTarget.title}」をクリアしました！</div>
            <div style={{fontSize:11,color:'var(--color-text-faint)',marginBottom:24}}>バッジは連続クリア日数で進化します</div>
            ...
            <button onClick={handleCloseModal} disabled={claiming} style={{width:'100%',padding:'12px',background:'var(--color-brand)',color:'var(--color-text-inverse)',border:'none',borderRadius:12,fontSize:15,fontWeight:700,cursor:claiming?'not-allowed':'pointer',opacity:claiming?0.6:1}}>
              {claiming ? '保存中...' : '受け取る'}
            </button>
          </div>
        </div>
