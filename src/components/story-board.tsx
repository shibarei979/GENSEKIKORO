...
import MemoMode from '@/components/memo-mode'
import CharacterMode from '@/components/character-mode'
import PlotMode from '@/components/plot-mode'
      ...
      <div style={{display:'flex',alignItems:'center',background:'var(--color-text)',padding:'0 8px',flexShrink:0}}>
          ...
          {TABS.map(t => (
            <button key={t.id} onClick={()=>setMode(t.id)}
              style={{
                ...
                color: mode===t.id ? 'var(--color-bg-card)' : 'color-mix(in srgb, var(--base-color-1) 60%, transparent)',
        ...
        {onClose && (
          <button onClick={onClose}
            style={{padding:'8px 16px',fontSize:13,border:'1px solid color-mix(in srgb, var(--color-text-inverse) 20%, transparent)',borderRadius:8,background:'color-mix(in srgb, var(--base-color-1) 8%, transparent)',color:'color-mix(in srgb, var(--base-color-1) 80%, transparent)',cursor:'pointer',marginRight:4}}>
            閉じる
          </button>
        )}
      </div>
