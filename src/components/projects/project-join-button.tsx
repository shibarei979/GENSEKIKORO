    ...
    <div>
      {!open ? (
        <button onClick={() => setOpen(true)}
          style={{ width: '100%', background: 'var(--color-brand)', color: 'var(--color-text-inverse)', border: 'none', borderRadius: 10, padding: '12px', fontSize: 14, fontWeight: 700, cursor: 'pointer' }}>
          この企画に参加する
        </button>
      ) : (
        <div style={{ background: 'var(--color-bg-card)', border: '1.5px solid var(--color-brand)', borderRadius: 12, padding: '16px' }}>
          ...
          <div style={{ display: 'flex', gap: 8 }}>
            ...
            <button onClick={handleJoin} disabled={saving || !selectedId} style={{ flex: 2, padding: '10px', border: 'none', borderRadius: 8, background: 'var(--color-brand)', color: 'var(--color-text-inverse)', fontSize: 13, fontWeight: 700, cursor: saving || !selectedId ? 'not-allowed' : 'pointer', opacity: saving || !selectedId ? 0.5 : 1 }}>
              {saving ? '参加中...' : '参加する'}
            </button>
          </div>
        </div>
