...
export default function StoryBoard({ userId, onClose, isModal }: Props) {
  ...
  return (
    <div style={{width:'100%',height:'100%',display:'flex',flexDirection:'column',background:'#f5f0ea',borderRadius:isModal?0:12,overflow:'hidden',fontFamily:"'Noto Sans JP',sans-serif"}}>
      ...
      {/* ===== タブ切り替えヘッダー ===== */}
      <div style={{display:'flex',alignItems:'center',background:'#2B211B',padding:'0 8px',flexShrink:0}}>
        <div style={{display:'flex',flex:1}}>
          {TABS.map(t => (
            <button key={t.id} onClick={()=>setMode(t.id)}
              style={{
                padding:'14px 22px', border:'none', cursor:'pointer',
                background: mode===t.id ? '#F26A21' : 'transparent',
                color: mode===t.id ? '#fff' : 'rgba(255,255,255,0.6)',
      ...
      {/* サブタイトル */}
      <div style={{padding:'8px 18px',background:'#fff',borderBottom:'1px solid #F0D9C9',fontSize:12,color:'#77706A',flexShrink:0}}>
