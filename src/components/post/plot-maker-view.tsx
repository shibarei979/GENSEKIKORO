      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
        {STEPS.map((step, i) => (
          <div key={step.key} style={{ display: 'flex', gap: 16, alignItems: 'flex-start' }}>
            <div style={{ width: 28, height: 28, borderRadius: '50%', background: data[step.key]?.trim() ? 'var(--color-brand)' : 'var(--color-brand-border)', color: data[step.key]?.trim() ? 'var(--base-color-1)' : 'var(--color-text-muted)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 12, fontWeight: 700, flexShrink: 0, marginTop: 4 }}>{i + 1}</div>
            <div style={{ flex: 1 }}>
