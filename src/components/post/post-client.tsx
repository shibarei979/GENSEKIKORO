...
import MemoSidebar, { type View } from '@/components/post/memo-sidebar'
import NoteCharacter from '@/components/post/note-character'
import NotePlot from '@/components/post/note-plot'
import NoteWorld from '@/components/post/note-world'
...
export default function PostClient({ profile, userId }: Props) {
  ...
  return (
    <div className="allow-select" style={{minHeight:'100vh'}}>
      <Header profile={profile} user={true} />
        ...
        {/* ビュー切り替え */}
        {currentView === 'character' && currentNovelId && <NoteCharacter novelId={currentNovelId} userId={userId||''}/>}
        {currentView === 'plot' && currentNovelId && <NotePlot novelId={currentNovelId} userId={userId||''}/>}
        {currentView === 'world' && currentNovelId && <NoteWorld novelId={currentNovelId} userId={userId||''}/>}
        {(currentView !== 'writing' && !currentNovelId) && (
          <div style={{display:'flex',flexDirection:'column',alignItems:'center',justifyContent:'center',height:'60vh',color:'var(--color-text-faint)',fontSize:14,gap:12}}>
            ...
            <button onClick={()=>setCurrentView('writing')} style={{background:'var(--color-brand)',color:'var(--color-text-inverse)',border:'none',borderRadius:8,padding:'8px 20px',fontSize:13,cursor:'pointer'}}>執筆へ</button>
          </div>
        )}
        {currentView === 'writing' && (
        ...
        {editMode && (
          <div style={{display:'flex',alignItems:'center',justifyContent:'space-between',gap:10,flexWrap:'wrap',marginBottom:14,background:'var(--color-brand-light)',border:'1px solid var(--color-brand-border)',borderRadius:10,padding:'10px 14px'}}>
            ...
            <button type="button" onClick={()=>{window.location.href=`/post?novel=${editNovelId}`}}
              style={{fontSize:12.5,fontWeight:700,color:'var(--color-text-inverse)',background:'var(--color-brand)',border:'none',borderRadius:8,padding:'8px 16px',cursor:'pointer',display:'inline-flex',alignItems:'center',gap:6}}>
                  ...
                  <div style={{position:'fixed',inset:0,background:'rgba(0,0,0,0.5)',zIndex:9999,display:'flex',alignItems:'center',justifyContent:'center',padding:isMobile?'16px':20}}
                    ...
                    <div onClick={e=>e.stopPropagation()} style={{
                      ...
                      <div style={{padding:'10px 14px',borderTop:'1px solid var(--color-brand-border)',background:'var(--color-bg)',display:'flex',gap:8,flexShrink:0}}>
                        ...
                        <div style={{flex:2,padding:'9px 0',background:'var(--color-brand)',color:'var(--color-text-inverse)',fontWeight:700,fontSize:14,borderRadius:8,textAlign:'center'}}>
                          作品を読む →
                        </div>
                      </div>
                    </div>
                ...
                <div style={{overflowX:'auto',scrollbarWidth:'none' as any,marginBottom:5}}>
                  <div style={{display:'flex',gap:3,padding:5,background:'var(--color-bg)',border:'1px solid var(--color-brand-border)',borderRadius:6,minWidth:'max-content'}}>
                    ...
                    <button type="button" onClick={()=>setShowPreview(true)}
                      style={{...toolBtn,background:'var(--color-brand)',color:'var(--color-text-inverse)',borderColor:'var(--color-brand)'}}>
                      プレビュー
                    </button>
                    <div style={{width:1,background:'var(--color-brand-border)',margin:'0 2px'}}/>
          ...
          <div style={{background:'var(--color-bg-card)',borderRadius:16,width:'100%',maxWidth:460,boxShadow:'0 8px 32px rgba(0,0,0,0.2)',overflow:'hidden'}}>
            ...
            <div style={{padding:'12px 20px',borderTop:'1px solid var(--color-brand-border)',display:'flex',gap:8}}>
              ...
              <button onClick={()=>{setShowReadabilityCheck(false);handleSubmit(pendingPublish)}}
                style={{flex:1,padding:'10px',background:'var(--color-brand)',color:'var(--color-text-inverse)',border:'none',borderRadius:8,fontSize:13,fontWeight:700,cursor:'pointer'}}>
                このまま投稿する
              </button>
            </div>
          </div>
        ...
        <div style={{position:'fixed',inset:0,background:'rgba(0,0,0,0.7)',zIndex:9999,display:'flex',flexDirection:'column',overflow:'hidden'}}
          ...
          <div style={{background:'var(--color-bg)',borderBottom:'1px solid var(--color-brand-border)',padding:'10px 16px',display:'flex',alignItems:'center',justifyContent:'space-between',flexShrink:0}}
            ...
            <button onClick={()=>setShowPreview(false)}
              style={{padding:'5px 14px',background:'var(--color-brand)',color:'var(--color-text-inverse)',border:'none',borderRadius:8,fontSize:13,cursor:'pointer'}}>
              閉じる
            </button>
          </div>
      ...
      {toast && (
        <div style={{position:'fixed',bottom: isMobile ? 80 : 24,right:24,background:'var(--color-brand)',color:'var(--color-bg-card)',padding:'12px 20px',borderRadius:12,fontSize:14,fontWeight:600,zIndex:9999,boxShadow:'0 4px 16px color-mix(in srgb, var(--color-brand) 35%, transparent)'}}>
          {toast}
        </div>
      )}
      {publishedInfo && (
        <div style={{position:'fixed',inset:0,background:'rgba(0,0,0,0.5)',zIndex:10000,display:'flex',alignItems:'center',justifyContent:'center',padding:16}}>
          <div style={{background:'var(--color-bg-card)',borderRadius:16,padding:'28px 24px',maxWidth:420,width:'100%',textAlign:'center'}}>
            ...
            <button onClick={()=>{
              ...
              style={{display:'block',width:'100%',fontSize:14,fontWeight:700,color:'var(--color-text-inverse)',background:'#000',border:'none',borderRadius:10,padding:'12px',cursor:'pointer',marginBottom:10}}>
              𝕏 でポストする
            </button>
