...
export default function TweetSection({ authorId, currentUserId, currentUserName, currentUserIconUrl, isOwner }: Props) {
  ...
  return (
    <div>
      ...
      {isOwner && (
        <div style={{background:'var(--color-bg-card)',border:'1px solid #F3ECE5',borderRadius:16,padding:'20px',marginBottom:20,boxShadow:'0 1px 3px rgba(0,0,0,0.02)'}}>
          <div style={{display:'flex',gap:12}}>
            ...
            <div style={{flex:1}}>
              <textarea
                ...
                style={{width:'100%',padding:'12px 14px',border:'1px solid #EADFD4',borderRadius:10,fontSize:14,resize:'none',outline:'none',fontFamily:'inherit',boxSizing:'border-box',lineHeight:1.7}}
              />
              {imagePreview && (
              ...
              <div style={{display:'flex',alignItems:'center',justifyContent:'space-between',marginTop:12}}>
                <div style={{display:'flex',alignItems:'center',gap:10}}>
                  <button onClick={()=>fileInputRef.current?.click()}
                    style={{display:'inline-flex',alignItems:'center',gap:6,height:36,padding:'0 14px',border:'1px solid #EADFD4',borderRadius:10,fontSize:13,color:'var(--color-text-muted)',background:'var(--color-bg-card)',cursor:'pointer'}}>
                    <IconImage/>
                    画像
                  </button>
                ...
                <button onClick={handlePost} disabled={posting||!body.trim()}
                  style={{height:40,padding:'0 24px',background:'var(--color-brand)',color:'#fff',border:'none',borderRadius:10,fontSize:14,fontWeight:600,cursor:'pointer',opacity:posting||!body.trim()?0.5:1}}>
                  {posting ? '投稿中...' : '投稿'}
                </button>
              </div>
            </div>
      ...
      {loading ? (
        ...
        isOwner ? (
          <div style={{textAlign:'center',padding:'56px 24px',color:'var(--color-text-faint)',fontSize:14,background:'var(--color-bg-card)',border:'1px solid #F3ECE5',borderRadius:16}}>
            まだつぶやきがありません。上の入力欄から投稿できます。
          </div>
        ) : null
      ) : tweets.map(tweet => (
        <div key={tweet.id} style={{background:'var(--color-bg-card)',border:'1px solid #F3ECE5',borderRadius:16,marginBottom:16,overflow:'hidden',boxShadow:'0 1px 3px rgba(0,0,0,0.02)'}}>
          <div style={{padding:'20px 22px'}}>
            ...
            <div style={{display:'flex',alignItems:'center',gap:10,paddingTop:12,borderTop:'1px solid #F7F2EC'}}>
              <button onClick={()=>handleLike(tweet.id, tweet.liked)}
                style={{display:'inline-flex',alignItems:'center',gap:6,height:34,padding:'0 14px',borderRadius:10,border:'1px solid',fontSize:13,cursor:currentUserId?'pointer':'default',
                  ...
                  borderColor:tweet.liked?'#FCA5A5':'#EADFD4',
              ...
              <button onClick={()=>setTweets(prev=>prev.map(t=>t.id===tweet.id?{...t,showComments:!t.showComments}:t))}
                style={{display:'inline-flex',alignItems:'center',gap:6,height:34,padding:'0 14px',borderRadius:10,border:'1px solid',fontSize:13,cursor:'pointer',
                  background:tweet.showComments?'#FFF0E5':'var(--color-bg-card)',
                  borderColor:tweet.showComments?'var(--color-brand)':'#EADFD4',
          ...
          {tweet.showComments && (
            <div style={{borderTop:'1px solid #F3ECE5',background:'var(--color-bg)'}}>
              {currentUserId && (
                <div style={{display:'flex',gap:10,padding:'14px 22px',alignItems:'center',borderBottom:'1px solid #F3ECE5',background:'var(--color-bg-card)'}}>
                  ...
                  <input
                    ...
                    placeholder="コメントを入力..."
                    maxLength={200}
                    style={{flex:1,height:38,padding:'0 14px',border:'1px solid #EADFD4',borderRadius:10,fontSize:13,outline:'none'}}
                  />
                  <button onClick={()=>handleComment(tweet.id)} disabled={commentPosting[tweet.id]||!commentBody[tweet.id]?.trim()}
                    style={{height:38,padding:'0 16px',background:'var(--color-brand)',color:'#fff',border:'none',borderRadius:10,fontSize:13,fontWeight:600,cursor:'pointer',opacity:commentPosting[tweet.id]||!commentBody[tweet.id]?.trim()?0.5:1}}>
                    送信
                  </button>
                </div>
              )}
              {tweet.comments.slice(0, tweet.showCount||5).map(c => (
                <div key={c.id} style={{display:'flex',gap:10,padding:'14px 22px',borderBottom:'1px solid #F7F2EC'}}>
