...
export default function ContestEntry({ novels, contests, initialEntries, userId }: Props) {
  ...
  return (
    <div style={{background:'var(--color-bg-card)',border:'1px solid var(--color-brand-border)',borderRadius:16,overflow:'hidden',marginTop:12}}>
      <div style={{padding:'14px 20px',borderBottom:'1px solid var(--color-brand-border)',background:'var(--color-bg)'}}>
        <span style={{fontSize:14,fontWeight:700,color:'var(--color-text)'}}>コンテスト応募</span>
      </div>

      {contests.length === 0 ? (
        <div style={{padding:'32px',textAlign:'center',color:'var(--color-text-faint)',fontSize:13}}>
          現在募集中のコンテストはありません
        </div>
      ) : (
        <div style={{padding:'16px 20px'}}>
          ...
          <div style={{display:'grid',gap:10,marginBottom:16}}>
            <div>
              <label style={{fontSize:11,color:'var(--color-text-muted)',fontWeight:600,display:'block',marginBottom:4}}>コンテストを選択</label>
              <select value={selectedContest} onChange={e=>{setSelectedContest(e.target.value);setSelectedNovel('');setError('')}}
                style={{width:'100%',padding:'8px 12px',border:'1px solid var(--color-brand-border)',borderRadius:8,fontSize:13,outline:'none',background:'var(--color-bg-card)'}}>
            ...
            {selectedContest && (
              <div>
                <label style={{fontSize:11,color:'var(--color-text-muted)',fontWeight:600,display:'block',marginBottom:4}}>応募する作品を選択</label>
                {publishedNovels.length === 0 ? (
                  <div style={{fontSize:12,color:'var(--color-text-faint)',padding:'8px 0'}}>公開中の作品がありません</div>
                ) : (
            ...
            {selectedContest && (
              <button onClick={handleEntry} disabled={loading||!selectedNovel}
                style={{padding:'8px 20px',background:'var(--color-brand)',color:'var(--color-text-inverse)',border:'none',borderRadius:8,fontSize:13,fontWeight:700,cursor:'pointer',opacity:loading||!selectedNovel?0.5:1,alignSelf:'flex-start'}}>
                {loading?'応募中...':'応募する'}
              </button>
            )}
          </div>
          ...
          {entries.length > 0 && (
            <div>
              <div style={{fontSize:12,fontWeight:700,color:'var(--color-text)',marginBottom:8}}>応募済み一覧</div>
              <div style={{display:'flex',flexDirection:'column',gap:6}}>
                {entries.map((e, i) => (
                  <div key={i} style={{display:'flex',alignItems:'center',gap:10,padding:'8px 12px',background:'var(--color-bg)',borderRadius:8,border:'1px solid var(--color-brand-border)'}}>
                    <div style={{flex:1,minWidth:0}}>
                      <div style={{fontSize:12,fontWeight:600,color:'var(--color-brand)',marginBottom:2}}>{contestMap[e.contest_id]?.title || 'コンテスト'}</div>
                      <div style={{fontSize:12,color:'var(--color-text)'}}>{novelMap[e.novel_id]?.title || '作品'}</div>
                    </div>
