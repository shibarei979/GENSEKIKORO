...
export default function FeatureFlagsManager({ features: initial }: { features: Feature[] }) {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
      {toast && (
        <div style={{ position: 'fixed', top: 20, left: '50%', transform: 'translateX(-50%)', background: '#1e293b', color: 'var(--color-text-inverse)', padding: '10px 20px', borderRadius: 8, fontSize: 13, zIndex: 100 }}>
          {toast}
        </div>
      )}
      {features.map(f => {
        ...
        return (
          <div key={f.key} style={{ background: 'var(--color-bg-card)', border: '1px solid #e2e8f0', borderRadius: 12, padding: '16px 18px' }}>
              ...
              {STATUS_OPTIONS.map(opt => (
                <button key={opt.value} onClick={() => updateStatus(f.key, f.label, opt.value)} disabled={saving === f.key}
                  style={{
                    ...
                    background: f.status === opt.value ? opt.bg : 'var(--base-color-1)',
