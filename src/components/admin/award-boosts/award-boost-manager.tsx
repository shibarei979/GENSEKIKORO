...
export default function AwardBoostManager({ boosts, novels }: { boosts: Boost[]; novels: NovelOpt[] }) {
  ...
  const card = { background: 'var(--color-bg-card)', border: '1px solid #e2e8f0', borderRadius: 12, padding: '18px 20px', marginBottom: 16 }
  const now = Date.now()

  return (
    <div>
      {msg && <div style={{ position: 'fixed', top: 20, left: '50%', transform: 'translateX(-50%)', background: '#1e293b', color: 'var(--color-text-inverse)', padding: '10px 20px', borderRadius: 8, fontSize: 13, zIndex: 100 }}>{msg}</div>}
      ...
      <div style={card}>
        ...
        {!selectedNovel ? (
          <div style={{ position: 'relative', marginBottom: 4 }}>
            ...
            {filtered.length > 0 && (
              <div style={{ position: 'absolute', top: '100%', left: 0, right: 0, background: 'var(--color-bg-card)', border: '1px solid #e2e8f0', borderRadius: 8, marginTop: 4, zIndex: 10, boxShadow: '0 4px 12px rgba(0,0,0,0.1)' }}>
                {filtered.map(n => (
        ...
        <div style={{ display: 'flex', gap: 8, marginTop: 10, marginBottom: 12, flexWrap: 'wrap' }}>
          {PRESETS.map((p, i) => (
            <button key={i} onClick={() => { setPreset(i); setCustomMult(''); setCustomDays('') }}
              style={{ flex: 1, minWidth: 120, padding: '10px', borderRadius: 8, cursor: 'pointer', border: preset === i ? '2px solid #2563eb' : '1px solid #e2e8f0', background: preset === i ? '#eff6ff' : 'var(--base-color-1)' }}>
        ...
        <button onClick={handleGrant} disabled={!selectedNovel || saving}
          style={{ fontSize: 13, fontWeight: 700, color: 'var(--color-text-inverse)', background: selectedNovel ? '#2563eb' : '#94a3b8', border: 'none', borderRadius: 8, padding: '10px 24px', cursor: selectedNovel ? 'pointer' : 'not-allowed' }}>
