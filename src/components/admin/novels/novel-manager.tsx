    ...
    <div>
      ...
      {notifyModal && (
        <div style={{position:'fixed',inset:0,background:'rgba(0,0,0,0.5)',zIndex:9999,display:'flex',alignItems:'center',justifyContent:'center',padding:16}}>
          <div style={{background:'var(--color-bg-card)',borderRadius:16,padding:'24px',maxWidth:420,width:'100%',boxShadow:'0 8px 32px rgba(0,0,0,0.2)'}}>
            ...
            <div style={{display:'flex',gap:8,justifyContent:'flex-end'}}>
              <button onClick={()=>{setNotifyModal(null);setNotifyResult('')}} style={btn('#64748b','var(--base-color-1)','#e2e8f0')}>
                {notifyResult ? '閉じる' : 'キャンセル'}
              </button>
              {!notifyResult && (
                <button onClick={handleNotifyFirst100} disabled={notifySending}
                  style={btn('var(--base-color-1)','var(--color-brand)','var(--color-brand)')}>
