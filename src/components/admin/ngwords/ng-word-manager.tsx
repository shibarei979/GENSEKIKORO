  ...
  return (
    <div>
      ...
      <div style={{display:'flex',gap:6,marginBottom:12}}>
        <button onClick={()=>setMode('single')} style={btn(mode==='single'?'var(--base-color-1)':'#64748b',mode==='single'?'var(--color-brand)':'var(--base-color-1)',mode==='single'?'var(--color-brand)':'#e2e8f0')}>
          1件ずつ追加
        </button>
        <button onClick={()=>setMode('bulk')} style={btn(mode==='bulk'?'var(--base-color-1)':'#64748b',mode==='bulk'?'var(--color-brand)':'var(--base-color-1)',mode==='bulk'?'var(--color-brand)':'#e2e8f0')}>
          まとめて追加
        </button>
      </div>

      {/* 入力エリア */}
      <div style={{background:'var(--color-bg-card)',border:'1px solid #e2e8f0',borderRadius:12,padding:'16px',marginBottom:16}}>
        {mode === 'single' ? (
          <div style={{display:'flex',gap:8}}>
            ...
            <button onClick={handleAdd} disabled={loading||!input.trim()}
              style={{...btn('var(--base-color-1)','var(--color-brand)','var(--color-brand)'),opacity:loading||!input.trim()?0.5:1}}>
              {loading?'追加中...':'追加'}
            </button>
          </div>
        ) : (
          <div>
            ...
            <div style={{display:'flex',justifyContent:'flex-end',marginTop:8}}>
              <button onClick={handleBulkAdd} disabled={loading||!bulk.trim()}
                style={{...btn('var(--base-color-1)','var(--color-brand)','var(--color-brand)'),opacity:loading||!bulk.trim()?0.5:1}}>
                {loading?'追加中...':'まとめて追加'}
              </button>
            </div>
          </div>
            ...
            {filtered.map(w => (
              <div key={w.id} style={{display:'inline-flex',alignItems:'center',gap:6,background:'#fef2f2',border:'1px solid #fca5a5',borderRadius:20,padding:'4px 12px'}}>
                ...
                <button onClick={()=>handleDelete(w.id)}
                  style={{width:16,height:16,borderRadius:'50%',border:'none',background:'#fca5a5',color:'var(--color-text-inverse)',cursor:'pointer',display:'flex',alignItems:'center',justifyContent:'center',fontSize:10,fontWeight:700,padding:0,lineHeight:1}}>
                  ×
                </button>
              </div>
            ))}
