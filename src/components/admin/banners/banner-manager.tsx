  ...
  return (
    <div>
      <div style={{display:'flex',justifyContent:'flex-end',marginBottom:16}}>
        <button onClick={openCreate} style={{...btn('var(--base-color-1)','var(--color-brand)','var(--color-brand)'),fontSize:13,padding:'8px 20px'}}>＋ バナーを追加</button>
      </div>
      ...
      {(creating || editing) && (
        <div style={{background:'var(--color-bg-card)',border:'1px solid #e2e8f0',borderRadius:12,padding:'24px',marginBottom:20}}>
          ...
          <div style={{display:'flex',gap:8,marginTop:16,justifyContent:'flex-end'}}>
            ...
            <button onClick={handleSave} disabled={loading||!form.title} style={btn('var(--base-color-1)','var(--color-brand)','var(--color-brand)')}>
              {loading?'保存中...':'保存する'}
            </button>
          </div>
        </div>
