...
const CHAR_COLORS = ['var(--color-brand)','#3b82f6','#e11d48','#22c55e','#8b5cf6','#f59e0b','#06b6d4','#ec4899']
const RELATION_COLORS = [
  { color:'#e11d48', label:'対立' },
  { color:'#ec4899', label:'恋愛' },
  { color:'#3b82f6', label:'友好' },
  { color:'#8b5cf6', label:'師弟' },
  { color:'var(--color-text-muted)', label:'その他' },
]
      ...
      <div ref={wrapRef} style={{flex:1,overflow:'hidden',position:'relative',cursor:mode==='select'?'default':'pointer',background:'#ddd4c4'}}>
        {ready && (
          <svg ref={svgRef} width="100%" height="100%" preserveAspectRatio="none"
            ...
            <rect x={0} y={0} width={BOARD_W} height={BOARD_H} fill="#fdfaf5" stroke="var(--color-brand)" strokeWidth={4} vectorEffect="non-scaling-stroke" style={{pointerEvents:'none',filter:'drop-shadow(0 2px 12px rgba(0,0,0,0.1))'}}/>
            ...
            {relations.map(rel=>{
              ...
              return (
                <g key={rel.id}>
                  ...
                  <rect x={mx-26} y={my-11} width={52} height={20} rx={10} fill="var(--base-color-1)" stroke={rel.color} strokeWidth={1.5} vectorEffect="non-scaling-stroke"/>
            ...
            {characters.map(c=>(
              <g key={c.id} onMouseDown={e=>handleCardMouseDown(e,c.id)} style={{cursor:mode==='select'?'move':'pointer'}}>
                <rect x={c.x} y={c.y} width={CARD_W} height={CARD_H} rx={10}
                  fill="var(--base-color-1)" stroke={selected===c.id?'var(--color-brand)':c.color} strokeWidth={selected===c.id?3:2.5}
                ...
                <text x={c.x+CARD_W/2} y={c.y+36} textAnchor="middle" fontSize={15} fontWeight={700} fill="var(--color-text)" fontFamily="'Noto Serif JP',serif" style={{userSelect:'none'}}>
                  {c.name.length>8?c.name.slice(0,8)+'…':c.name}
                </text>
                {c.memo && (
                  <text x={c.x+CARD_W/2} y={c.y+58} textAnchor="middle" fontSize={10} fill="var(--color-text-muted)" fontFamily="'Noto Sans JP',sans-serif" style={{userSelect:'none'}}>
                    {c.memo.length>14?c.memo.slice(0,14)+'…':c.memo}
                  </text>
                )}
                {connectFromRef.current===c.id && (
                  <rect x={c.x-4} y={c.y-4} width={CARD_W+8} height={CARD_H+8} rx={12} fill="none" stroke="var(--color-brand)" strokeWidth={2.5} vectorEffect="non-scaling-stroke" strokeDasharray="6 3" style={{pointerEvents:'none'}}/>
                )}
              </g>
            ))}
          </svg>
        ...
        <div style={{position:'absolute',inset:0,zIndex:100,display:'flex',alignItems:'center',justifyContent:'center',background:'rgba(0,0,0,0.4)'}}>
          <div style={{background:'var(--color-bg-card)',borderRadius:14,padding:'22px',width:300,boxShadow:'0 8px 32px rgba(0,0,0,0.2)'}}>
            ...
            <div style={{display:'flex',gap:8}}>
              ...
              <button onClick={()=>customLabel.trim()&&confirmRelation(customLabel.trim(),'var(--color-text-muted)')}
