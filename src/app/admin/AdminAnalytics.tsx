...
const COLORS = ['#F26A21','#3b82f6','#10b981','#f59e0b','#8b5cf6','#ec4899','#14b8a6','#f97316','#6366f1','#84cc16']

const CustomTooltip = ({ active, payload, label }: any) => {
  if (!active || !payload?.length) return null
  return (
    <div style={{background:'#fff',border:'1px solid #e2e8f0',borderRadius:8,padding:'8px 12px',fontSize:12,boxShadow:'0 2px 8px rgba(0,0,0,0.1)'}}>
...
export default function AdminAnalytics({
  ...
  const tabBtn = (id: string) => ({
    ...
    background: tab===id ? '#F26A21' : '#fff',
    color: tab===id ? '#fff' : '#64748b',
    borderColor: tab===id ? '#F26A21' : '#e2e8f0',
    ...
    <div style={{background:'#fff',border:'1px solid #e2e8f0',borderRadius:12,padding:'20px',marginBottom:20}}>
            ...
            <div>
              ...
              <ResponsiveContainer width="100%" height={220}>
                <BarChart data={genreStats.sort((a,b)=>b.likes-a.likes)} layout="vertical" margin={{left:40,right:10}}>
                  ...
                  <Bar dataKey="likes" name="いいね数" fill="#F26A21" radius={[0,3,3,0]}/>
                </BarChart>
              </ResponsiveContainer>
            </div>
          </div>
          ...
          <table style={{width:'100%',borderCollapse:'collapse',marginTop:14,fontSize:12}}>
            ...
            <tbody>
              {genreStats.sort((a,b)=>b.count-a.count).map((g,i)=>(
                <tr key={g.genre} style={{borderBottom:'1px solid #f1f5f9',background:i%2===0?'#fff':'#fafafa'}}>
                  ...
                  <td style={{padding:'7px 12px',color:'#F26A21',fontWeight:700}}>{g.likes.toLocaleString()}</td>
            ...
            <BarChart data={hourlyAccess} margin={{top:10,right:10,left:-20,bottom:0}}>
              ...
              <Bar dataKey="count" name="PV数" fill="#3b82f6" radius={[3,3,0,0]}>
                {hourlyAccess.map((d,i)=>{
                  ...
                  return <Cell key={i} fill={d.count===peak?'#F26A21':d.count>peak*0.7?'#f59e0b':'#3b82f6'}/>
                })}
              </Bar>
            </BarChart>
          </ResponsiveContainer>
          ...
          {(() => {
            ...
            return (
              <div style={{display:'flex',gap:16,marginTop:12,padding:'12px 16px',background:'#f8fafc',borderRadius:8,fontSize:12}}>
                <div><span style={{color:'#64748b'}}>ピーク時間：</span><strong style={{color:'#F26A21'}}>{peak.hour}時台</strong>（{peak.count.toLocaleString()} PV）</div>
            ...
            <tbody>
              {topNovels.sort((a,b)=>b.views-a.views).map((n,i)=>(
                <tr key={n.id} style={{borderBottom:'1px solid #f1f5f9',background:i%2===0?'#fff':'#fafafa'}}>
                  ...
                  <td style={{padding:'7px 12px'}}><span style={{fontSize:10,background:'#FFF1E6',color:'#F26A21',padding:'1px 6px',borderRadius:3}}>{n.genre}</span></td>
                  <td style={{padding:'7px 12px',color:'#3b82f6',fontWeight:700}}>{n.views.toLocaleString()}</td>
                  <td style={{padding:'7px 12px',color:'#F26A21',fontWeight:700}}>{n.likes.toLocaleString()}</td>
                </tr>
              ))}
            </tbody>
          </table>
          ...
          <div style={{marginTop:14,display:'flex',flexWrap:'wrap',gap:8}}>
            {missionStats.sort((a,b)=>b.count-a.count).map(m=>(
              <div key={m.mission_id} style={{background:'#f8fafc',border:'1px solid #e2e8f0',borderRadius:8,padding:'8px 12px',fontSize:11}}>
                ...
                <div style={{fontWeight:700,color:'#F26A21',fontSize:16}}>{m.count}<span style={{fontSize:11,fontWeight:400,color:'#64748b'}}> 人</span></div>
              </div>
            ))}
          </div>
        </div>
      ...
      {tab==='speech' && (
        <div>
          ...
          <div style={{display:'flex',gap:20,marginBottom:20}}>
            ...
            <div style={{flex:1,display:'flex',flexDirection:'column',justifyContent:'center',gap:12}}>
              {[
                ...
                {label:'読み上げ利用者',value:speechStats.used,color:'#F26A21'},
          ...
          <div style={{padding:'12px 16px',background:'#FFF9F2',border:'1px solid #F0D9C9',borderRadius:8,fontSize:12,color:'#77706A',lineHeight:1.7}}>
          ...
          <ResponsiveContainer width="100%" height={220}>
            <BarChart data={pageViewStats.slice(0,15)} layout="vertical" margin={{left:100,right:30}}>
              ...
              <Bar dataKey="count" name="PV数" fill="#10b981" radius={[0,3,3,0]}>
                {pageViewStats.slice(0,15).map((_,i)=><Cell key={i} fill={i===0?'#F26A21':i<3?'#f59e0b':'#10b981'}/>)}
              </Bar>
            </BarChart>
          </ResponsiveContainer>
          <table style={{width:'100%',borderCollapse:'collapse',marginTop:14,fontSize:12}}>
            ...
            <tbody>
              {pageViewStats.map((p,i)=>(
                <tr key={p.path} style={{borderBottom:'1px solid #f1f5f9',background:i%2===0?'#fff':'#fafafa'}}>
                  <td style={{padding:'7px 12px',color:'#94a3b8',fontWeight:700}}>{i+1}</td>
                  ...
                  <td style={{padding:'7px 12px'}}>
                    <div style={{display:'flex',alignItems:'center',gap:6}}>
                      <div style={{flex:1,height:4,background:'#f1f5f9',borderRadius:2,overflow:'hidden',minWidth:60}}>
                        <div style={{height:'100%',background:i===0?'#F26A21':'#10b981',borderRadius:2,width:`${totalPageViews>0?(p.count/totalPageViews*100):0}%`}}/>
                      </div>
