    ...
    <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:16,alignItems:'start'}}>
        ...
        <div style={{display:'flex',gap:6,marginBottom:12}}>
          {(['all','unread','read'] as const).map(f => (
            <button key={f} onClick={()=>setFilter(f)}
              style={{...btn(filter===f?'#fff':'#64748b', filter===f?'#F26A21':'#fff', filter===f?'#F26A21':'#e2e8f0'),fontSize:11}}>
          ...
          {filtered.length === 0 ? (
            ...
            <div key={c.id}
              ...
              style={{
                ...
                cursor:'pointer',
                background: selected?.id === c.id ? '#FFF1E6' : c.is_read ? '#fff' : '#fefce8',
                borderLeft: selected?.id === c.id ? '3px solid #F26A21' : '3px solid transparent',
              }}>
      ...
      <div>
        {selected ? (
              ...
              <div>
                ...
                <div style={{display:'flex',justifyContent:'flex-end',marginTop:6}}>
                  <button onClick={handleSaveNote} disabled={saving}
                    style={{...btn('#fff','#F26A21','#F26A21'),opacity:saving?0.6:1}}>
                    {saving?'保存中...':'メモを保存'}
                  </button>
                </div>
              </div>
          ...
          <div style={{background:'#fff',border:'1px solid #e2e8f0',borderRadius:12,padding:'48px',textAlign:'center',color:'#94a3b8',fontSize:13}}>
