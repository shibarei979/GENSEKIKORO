      ...
      <div>
        <div style={{background:'#fff',border:'1px solid #e2e8f0',borderRadius:12,padding:'20px',marginBottom:16}}>
              ...
              {filteredUsers.slice(0,30).map(u => (
                <div key={u.user_id}
                  ...
                  style={{
                    ...
                    background: toUserId===u.user_id ? '#FFF1E6' : '#fff',
                    borderBottom:'1px solid #f1f5f9',
                  }}>
                  {u.icon_url
                    ? <img src={u.icon_url} style={{width:22,height:22,borderRadius:'50%',objectFit:'cover'}} alt=""/>
                    : <div style={{width:22,height:22,borderRadius:'50%',background:'#F0D9C9',display:'flex',alignItems:'center',justifyContent:'center',fontSize:10,color:'#F26A21',fontWeight:700}}>{u.display_name?.[0]}</div>
                  }
                  <div>
                    <div style={{fontWeight:600,color: toUserId===u.user_id?'#F26A21':'#1e293b'}}>{u.display_name}</div>
          ...
          <button onClick={handleSend} disabled={loading}
            style={{...btn('#fff','#F26A21','#F26A21'),width:'100%',padding:'10px',fontSize:13,opacity:loading?0.6:1}}>
            {loading?'送信中...':'送信する'}
          </button>
        </div>
      </div>
          ...
          {messages.length === 0 ? (
            ...
            <div key={m.id}
              ...
              style={{padding:'12px 16px',borderBottom:idx<messages.length-1?'1px solid #f1f5f9':'none',cursor:'pointer',
                background:selected?.id===m.id?'#FFF1E6':'#fff'}}>
