...
export default function AiReviewSection({ reviews: initialReviews }: Props) {
  ...
  return (
    <div style={{background:'#fff',border:'1px solid #e2e8f0',borderRadius:12,marginBottom:20,overflow:'hidden'}}>
              ...
              <div style={{display:'flex',alignItems:'flex-start',justifyContent:'space-between',gap:12,flexWrap:'wrap'}}>
                ...
                <div style={{display:'flex',flexDirection:'column',gap:6,flexShrink:0}}>
                  ...
                  <button onClick={()=>handleDelete(r)} disabled={loading===r.id}
                    style={{padding:'6px 14px',border:'1px solid #dc2626',borderRadius:8,background:'#dc2626',color:'#fff',fontSize:12,fontWeight:700,cursor:'pointer',opacity:loading===r.id?0.5:1,whiteSpace:'nowrap'}}>
                    🗑 完全削除
                  </button>
                </div>
              </div>
      ...
      {/* トースト */}
      {toast && (
        <div style={{position:'fixed',bottom:24,right:24,background:'#1e293b',color:'#fff',padding:'12px 20px',borderRadius:10,fontSize:13,fontWeight:600,zIndex:9999,boxShadow:'0 4px 16px rgba(0,0,0,0.2)'}}>
          {toast}
        </div>
      )}
    </div>
