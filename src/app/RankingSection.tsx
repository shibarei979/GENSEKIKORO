...
export default function RankingSection({ rankingLong, rankingShort, risingNovels }: Props) {
  const [tab, setTab] = useState<'weekly' | 'rising'>('weekly')
  ...
  return (
    <>
      {/* タブ */}
      <div style={{display:'flex',borderBottom:'1px solid #F0D9C9',background:'#FFF9F2'}}>
        <button onClick={()=>setTab('weekly')}
          style={{flex:1,padding:'8px',fontSize:12,fontWeight:700,border:'none',cursor:'pointer',
            background:tab==='weekly'?'#fff':'#FFF9F2',
            color:tab==='weekly'?'#F26A21':'#77706A',
            borderBottom:tab==='weekly'?'2px solid #F26A21':'2px solid transparent'}}>
          週間ランキング
        </button>
        <button onClick={()=>setTab('rising')}
          style={{flex:1,padding:'8px',fontSize:12,fontWeight:700,border:'none',cursor:'pointer',
            background:tab==='rising'?'#fff':'#FFF9F2',
            color:tab==='rising'?'#F26A21':'#77706A',
            borderBottom:tab==='rising'?'2px solid #F26A21':'2px solid transparent'}}>
          🔥 急上昇
        </button>
      </div>
            ...
            <NovelPreviewPopup key={n.id} novel={n}>
              <div style={{...rowStyle,cursor:'pointer'}}>
                ...
                <div style={{flex:1}}>
                  <div style={{marginBottom:2}}><span style={{fontSize:9,background:'#FFF1E6',color:'#F26A21',border:'1px solid #f5b080',padding:'1px 5px',borderRadius:3}}>{n.genre}</span></div>
                  <div style={{fontSize:13,fontWeight:700,color:'#2B211B'}}>{n.title}</div>
                  <div style={{fontSize:10,color:'#77706A',marginTop:1}}>作者：{n.display_name}</div>
                </div>
                <div style={{fontSize:10,color:'#F26A21',fontWeight:700,flexShrink:0}}>🔥 {n.rising_score}</div>
              </div>
            </NovelPreviewPopup>
          ))}
        </div>
