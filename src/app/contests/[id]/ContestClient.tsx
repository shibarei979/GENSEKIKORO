    ...
    <div style={{background:'#fff',border:'1px solid #F0D9C9',borderRadius:16,overflow:'hidden'}}>
      ...
      <div style={{padding:'12px 16px',borderBottom:'1px solid #F0D9C9',background:'#FFF9F2'}}>
        <div style={{display:'flex',alignItems:'center',justifyContent:'space-between',marginBottom:10,gap:8}}>
          <span style={{fontSize:14,fontWeight:700,color:'#2B211B',flexShrink:0}}>
            応募作品
            <span style={{fontSize:12,fontWeight:400,color:'#77706A',marginLeft:6}}>（{filtered.length}作品）</span>
          </span>
          {/* 並び順：横スクロール */}
          <div style={{overflowX:'auto',scrollbarWidth:'none' as any,msOverflowStyle:'none' as any}}>
      ...
      {filtered.length === 0 ? (
        ...
        <div style={{cursor:'pointer'}}>
          <div style={{padding:'12px 16px',borderBottom:i<filtered.length-1?'1px solid #FFF1E6':'none',
            background:myEntryNovelIds.includes(n.id)?'#FFF9F2':'#fff'}}>
            <div style={{display:'flex',gap:5,marginBottom:5,flexWrap:'wrap',alignItems:'center'}}>
              ...
              {n.is_serial
                ? <span style={{fontSize:10,background:'#f0fdf4',color:'#15803d',border:'1px solid #86efac',padding:'1px 5px',borderRadius:3}}>連載中</span>
                : <span style={{fontSize:10,background:'#f5f5f5',color:'#757575',border:'1px solid #e0e0e0',padding:'1px 5px',borderRadius:3}}>完結</span>
              }
              {myEntryNovelIds.includes(n.id) && (
                <span style={{fontSize:10,background:'#FFF1E6',color:'#F26A21',border:'1px solid #f5b080',padding:'1px 5px',borderRadius:3}}>自分の応募</span>
              )}
            </div>
            <div style={{fontSize:14,fontWeight:700,color:'#2B211B',marginBottom:2,lineHeight:1.4}}>{n.title}</div>
            <div style={{fontSize:11,color:'#77706A',marginBottom:n.summary?5:0}}>
              作者：{n.display_name} · ♡ {n.like_count}
            </div>
            {n.summary && (
              <div style={{fontSize:12,color:'#5a3a20',lineHeight:1.7,overflow:'hidden',display:'-webkit-box',
