                        ...
                        <div className="desktop-only" style={{display:'flex',gap:0}}>
                          ...
                          <div style={{padding:'20px 24px',flex:1,minWidth:0}}>
                            <div style={{display:'flex',alignItems:'center',gap:8,marginBottom:8,flexWrap:'wrap'}}>
                              ...
                              <span style={{fontSize:11,color:'#77706A',marginLeft:'auto'}}>応募数：{entryCount}作品</span>
                            </div>
                            <h3 style={{fontSize:17,fontWeight:700,color:'#2B211B',marginBottom:8,lineHeight:1.4,fontFamily:"'Noto Serif JP',serif"}}>{c.title}</h3>
                            {c.description && <p style={{fontSize:12,color:'#77706A',lineHeight:1.8,overflow:'hidden',display:'-webkit-box',WebkitLineClamp:2,WebkitBoxOrient:'vertical' as any}}>{c.description}</p>}
                            {status.label === '募集中' && (
                              <div style={{marginTop:10}}>
                                <span style={{display:'inline-block',padding:'6px 16px',background:'#F26A21',color:'#fff',fontWeight:700,fontSize:12,borderRadius:6}}>応募受付中 →</span>
                              </div>
                            )}
                          </div>
                        </div>
                        ...
                        <div className="mobile-only">
                          ...
                          <div style={{padding:'14px 16px'}}>
                            <div style={{display:'flex',alignItems:'center',gap:8,marginBottom:6,flexWrap:'wrap'}}>
                              <span style={{fontSize:11,fontWeight:700,color:status.color,background:status.bg,border:`1px solid ${status.border}`,padding:'2px 10px',borderRadius:10}}>{status.label}</span>
                              {c.deadline && <span style={{fontSize:11,color:'#94a3b8'}}>締切：{new Date(c.deadline).toLocaleDateString('ja-JP')}</span>}
                              <span style={{fontSize:11,color:'#77706A'}}>応募数：{entryCount}作品</span>
                            </div>
                            <h3 style={{fontSize:15,fontWeight:700,color:'#2B211B',marginBottom:6,lineHeight:1.4,fontFamily:"'Noto Serif JP',serif"}}>{c.title}</h3>
                            {c.description && <p style={{fontSize:12,color:'#77706A',lineHeight:1.8,overflow:'hidden',display:'-webkit-box',WebkitLineClamp:2,WebkitBoxOrient:'vertical' as any,margin:0}}>{c.description}</p>}
                            {status.label === '募集中' && <div style={{marginTop:10}}><span style={{display:'inline-block',padding:'6px 16px',background:'#F26A21',color:'#fff',fontWeight:700,fontSize:12,borderRadius:6}}>応募受付中 →</span></div>}
                          </div>
                        </div>
                      </div>
                    </Link>
                  ...
                  return (
                    <a key={c.id} href={c.apply_url||'#'} target="_blank" rel="noopener noreferrer" style={{textDecoration:'none'}}>
                      <div style={{background:'#fff',border:'1px solid #F0D9C9',borderRadius:12,overflow:'hidden'}}>
                        {/* デスクトップ */}
                        <div className="desktop-only" style={{display:'flex',gap:0}}>
                          {c.image_url && <div style={{width:200,flexShrink:0,overflow:'hidden',background:'#FFF9F2'}}><img src={c.image_url} alt={c.title} style={{width:'100%',height:'100%',objectFit:'cover',display:'block'}}/></div>}
                          <div style={{padding:'20px 24px',flex:1,minWidth:0}}>
                            <div style={{display:'flex',alignItems:'center',gap:8,marginBottom:8,flexWrap:'wrap'}}>
                              <span style={{fontSize:11,fontWeight:700,color:status.color,background:status.bg,border:`1px solid ${status.border}`,padding:'2px 10px',borderRadius:10}}>{status.label}</span>
                              {c.deadline && <span style={{fontSize:11,color:'#94a3b8'}}>締切：{new Date(c.deadline).toLocaleDateString('ja-JP')}</span>}
                        ...
                        <div className="mobile-only">
                          ...
                          <div style={{padding:'14px 16px'}}>
                            ...
                            <h3 style={{fontSize:15,fontWeight:700,color:'#2B211B',marginBottom:4,lineHeight:1.4,fontFamily:"'Noto Serif JP',serif"}}>{c.title}</h3>
                            {c.description && <p style={{fontSize:12,color:'#77706A',lineHeight:1.8,overflow:'hidden',display:'-webkit-box',WebkitLineClamp:2,WebkitBoxOrient:'vertical' as any,margin:0}}>{c.description}</p>}
                            {status.label === '募集中' && <div style={{marginTop:10}}><span style={{display:'inline-block',padding:'6px 16px',background:'#3b82f6',color:'#fff',fontWeight:700,fontSize:12,borderRadius:6}}>外部サイトで応募する ↗</span></div>}
                          </div>
                        </div>
