        ...
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.55)', zIndex: 300, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 10 }} onClick={() => setShowEditor(false)}>
          <div onClick={e => e.stopPropagation()} style={{ background: 'var(--color-bg-card)', borderRadius: 14, padding: '18px 16px', maxWidth: 640, width: '100%', maxHeight: '92vh', overflowY: 'auto' }}>
            ...
            <div
              ...
              style={{ display: 'grid', gridTemplateColumns: `repeat(${W}, 1fr)`, border: '1px solid var(--color-brand-border)', borderRadius: 6, overflow: 'hidden', touchAction: 'none', userSelect: 'none', background: '#fff', aspectRatio: `${W} / ${H}` }}>
              {dots.map((c, i) => (
                <div key={i} data-idx={i}
                  onMouseDown={() => paint(i)}
                  onMouseEnter={() => { if (paintingRef.current) paint(i) }}
                  style={{ background: c > 0 ? OBI_PALETTE[c] : ((Math.floor(i / W) + (i % W)) % 2 === 0 ? '#ffffff' : '#f7f4f0'), aspectRatio: '1', cursor: 'crosshair' }} />
              ))}
            </div>
            ...
            <div style={{ display: 'flex', gap: 8, justifyContent: 'flex-end' }}>
              ...
              <button onClick={handleSave} disabled={saving}
                style={{ fontSize: 13, fontWeight: 700, color: '#fff', background: 'var(--color-brand)', border: 'none', borderRadius: 8, padding: '9px 22px', cursor: 'pointer' }}>
                {saving ? '保存中...' : '保存する'}
              </button>
            </div>
          </div>
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginTop: 6, flexWrap: 'wrap', gap: 6 }}>
                    ...
                    <div style={{ display: 'flex', gap: 6 }}>
                      ...
                      <button onClick={() => handleApprove(o.id, true)}
                        style={{ fontSize: 12, fontWeight: 700, color: '#fff', background: 'var(--color-brand)', border: 'none', borderRadius: 8, padding: '6px 16px', cursor: 'pointer' }}>承認して表示</button>
                    </div>
                  </div>
                </div>
              ))
