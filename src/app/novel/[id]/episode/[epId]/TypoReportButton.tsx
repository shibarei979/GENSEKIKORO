            ...
            <>
              ...
              <div style={{ display: 'flex', gap: 8, justifyContent: 'flex-end' }}>
                ...
                <button onClick={handleSubmit} disabled={sending || !original.trim()}
                  style={{ fontSize: 12, fontWeight: 700, color: '#fff', background: 'var(--color-brand)', border: 'none', borderRadius: 8, padding: '7px 16px', cursor: sending || !original.trim() ? 'not-allowed' : 'pointer', opacity: sending || !original.trim() ? 0.5 : 1 }}>
                  {sending ? '送信中...' : '報告する'}
                </button>
              </div>
            </>
