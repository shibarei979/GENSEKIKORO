...
export default function ReadingSettings({ onChange, isMobile = false }: Props) {
  ...
  const btnBase = (active: boolean) => ({
    ...
    border: active ? '1.5px solid #F26A21' : '1px solid #F0D9C9',
    background: active ? '#FFF1E6' : '#fff',
    color: active ? '#F26A21' : '#77706A',
  })
    ...
    <div style={{position:'relative', display:'inline-block'}}>
      <button
        ...
        style={{
          ...
          border:'1px solid #F0D9C9', background:'#fff',
          fontSize:12, color:'#77706A', cursor:'pointer',
        }}
      >
      ...
      {open && (
        <>
          ...
          <div style={{
            ...
            {/* 文字サイズ */}
            <div style={{marginBottom:14}}>
              <div style={{fontSize:11,color:'#77706A',fontWeight:600,marginBottom:6}}>文字サイズ</div>
              <div style={{display:'flex',gap:6}}>
                {SIZE_OPTIONS.map(o => (
            ...
            {/* 行間 */}
            <div>
              <div style={{fontSize:11,color:'#77706A',fontWeight:600,marginBottom:6}}>行間</div>
              <div style={{display:'flex',gap:6}}>
                {LINE_OPTIONS.map(o => (
