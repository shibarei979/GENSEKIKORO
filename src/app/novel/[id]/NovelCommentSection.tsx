...
export default function NovelCommentSection({ novelId, userId, userName, userIconUrl, authorId }: Props) {
  ...
  function CommentCard({ c, isReply=false, parentId, parentRootId }: { c:Comment; isReply?:boolean; parentId?:string; parentRootId?:string }) {
    ...
    return (
      <div style={{
        ...
        {c.is_pinned && !isReply && (
          <div style={{fontSize:10,color:'var(--color-warning)',marginBottom:4}}>📌 ピン留め</div>
        )}
        ...
        <div style={{fontSize:12,color:'var(--color-text)',lineHeight:1.7,paddingLeft:isReply?29:33,whiteSpace:'pre-wrap'}}>
          {c.body}
        </div>
  ...
  return (
    <div style={{background:'var(--color-bg-card)',border:'1px solid var(--color-brand-border)',borderRadius:12,overflow:'hidden'}}>
      <div style={{padding:'12px 16px',borderBottom:'1px solid var(--color-brand-border)',display:'flex',alignItems:'center',justifyContent:'space-between'}}>
        ...
        {userId ? (
          <>
            ...
            <div style={{display:'flex',justifyContent:'flex-end',gap:8,alignItems:'center'}}>
              {error && <span style={{fontSize:11,color:'var(--color-danger)',marginRight:'auto'}}>{error}</span>}
