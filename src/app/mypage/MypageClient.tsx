  ...
  const BookmarksTab = () => {
    ...
    <div>
      ...
      <div style={{display:'flex',gap:24,alignItems:'flex-start',flexWrap:'wrap'}}>
        {/* 左：リストパネル */}
        <div style={{flex:'0 1 260px',minWidth:220,background:'var(--color-bg-card)',border:'1px solid #F3ECE5',borderRadius:16,padding:'20px 8px',boxShadow:'0 1px 3px rgba(0,0,0,0.02)'}}>
        ...
        <div style={{flex:'1 1 520px',minWidth:300}}>
          ...
          {listed.length === 0 ? (
            <div style={{textAlign:'center',padding:'64px 24px',color:'var(--color-text-faint)',fontSize:14,
              background:'var(--color-bg-card)',border:'1px solid #F3ECE5',borderRadius:20}}>
              このリストに作品がありません
            </div>
          ) : (
            <div style={{display:'flex',flexDirection:'column',gap:16}}>
              {listed.map((bm:any) => {
                ...
                return (
                  <div key={bm.novel_id}
                    style={{background:'var(--color-bg-card)',border:'1px solid #F3ECE5',borderRadius:20,padding:'24px 28px',
                    ...
                    <div style={{display:'flex',flexDirection:'column',gap:10,alignItems:'stretch'}}>
                      <Link href={`/novel/${n.id}`}
                        style={{height:44,display:'inline-flex',alignItems:'center',justifyContent:'center',background:'var(--color-brand)',color:'#fff',
    ...
    <div>
      ...
      <div style={{display:'flex',gap:24,alignItems:'flex-start',flexWrap:'wrap'}}>
        ...
        <div style={{flex:'0 1 300px',minWidth:260,display:'flex',flexDirection:'column',gap:16}}>
          {/* よく絡む作者 */}
          {followingAuthors.length > 0 ? (
            <div style={{background:'var(--color-bg-card)',border:'1px solid #F3ECE5',borderRadius:16,padding:'20px 22px',boxShadow:'0 1px 3px rgba(0,0,0,0.02)'}}>
      ...
      <div style={{width:'100%',padding:'0'}}>
        {isMobile ? (
          ...
          <div style={{display:'flex',minHeight:'calc(100vh - 60px)'}}>
            {/* 左サイドナビ */}
            <div style={{width:232,flexShrink:0,borderRight:'1px solid #F3ECE5',padding:'24px 12px',position:'sticky',top:60,height:'calc(100vh - 60px)',background:'var(--color-bg-card)',overflowY:'auto'}}>
              {visibleTabs.map(tab => {
              ...
              <div style={{marginTop:28,background:'#FFF8F1',border:'1px solid #F5DFC8',borderRadius:14,padding:'18px 16px',position:'relative',overflow:'hidden'}}>
                ...
                <svg width="56" height="56" viewBox="0 0 56 56" fill="none" style={{position:'absolute',right:-4,bottom:-6,opacity:0.5}}>
                  <path d="M40 14c-8 2-16 9-19 17-2 5-3 10-2 15l-4 4a1.5 1.5 0 0 0 2 2l4-4c5 1 10 0 15-2 8-3 15-11 17-19 1-4 2-9 2-12 0-1-1-2-2-2-3 0-8 1-13 1z"
                    fill="#FFE8D5" stroke="#F26A21" strokeWidth="1.5" strokeLinejoin="round"/>
                </svg>
              </div>
            </div>
            {/* 右コンテンツ */}
