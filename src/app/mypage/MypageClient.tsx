...
export default function MypageClient({
  ...
  const MypageTab = () => (
    <div>
      {!profile.birthdate && (
        <div style={{background:'var(--color-brand-light)',border:'1px solid #f5b080',borderRadius:10,padding:'12px 16px',display:'flex',alignItems:'center',gap:12,marginBottom:20}}>
      ...
      <div style={{display:'flex',alignItems:'flex-start',gap:20,marginBottom:24}}>
        <div style={{position:'relative',flexShrink:0,cursor:'pointer'}} onClick={()=>iconInputRef.current?.click()}>
          ...
          <div style={{position:'absolute',bottom:0,right:0,width:20,height:20,background:'#fff',borderRadius:'50%',border:'2px solid var(--color-brand)',display:'flex',alignItems:'center',justifyContent:'center',fontSize:10}}>
            {iconUploading?'⟳':'📷'}
          </div>
        </div>
        <div style={{flex:1}}>
            ...
            {editingName ? (
              <div style={{display:'flex',alignItems:'center',gap:8,flexWrap:'wrap'}}>
                <input value={nameInput} onChange={e=>setNameInput(e.target.value)}
                  ...
                  style={{fontSize:18,fontWeight:700,border:'1.5px solid var(--color-brand)',borderRadius:6,padding:'3px 10px',outline:'none',width:180}} autoFocus/>
          ...
          <div style={{display:'flex',gap:20,flexWrap:'wrap',marginBottom:8}}>
            <span style={{fontSize:13,color:'var(--color-text-muted)'}}><strong style={{color:'var(--color-text)',fontSize:16}}>{followerCount}</strong> フォロワー</span>
            <span style={{fontSize:13,color:'var(--color-text-muted)'}}><strong style={{color:'var(--color-text)',fontSize:16}}>{followingCount}</strong> フォロー中</span>
            <span style={{fontSize:13,color:'var(--color-brand)'}}><strong style={{fontSize:16}}>{published.length}</strong> 公開作品</span>
            <span style={{fontSize:13,color:'var(--color-text-muted)'}}><strong style={{color:'var(--color-text)',fontSize:16}}>{drafts.length}</strong> 下書き</span>
          </div>
          <div style={{fontSize:13,color:'var(--color-text-muted)',marginBottom:10}}>{profile.email}</div>
          {profile.bio && <div style={{fontSize:13,color:'var(--color-text)',lineHeight:1.8,marginBottom:10}}>{profile.bio}</div>}
          <div style={{display:'flex',gap:8,flexWrap:'wrap'}}>
            ...
            <button onClick={()=>setShowBoard(true)} style={{fontSize:12,border:'1px solid var(--color-brand-border)',padding:'5px 12px',borderRadius:8,background:'none',color:'var(--color-text-muted)',cursor:'pointer'}}>ストーリーボード</button>
          </div>
        </div>
      </div>
      ...
      {claimedSet.size > 0 && (
        <div style={{marginBottom:24}}>
          <div style={{fontSize:13,fontWeight:700,color:'var(--color-text)',marginBottom:10}}>獲得バッジ（{claimedSet.size}/{ALL_BADGES.filter(b=>!b.id.startsWith('_')).length}）</div>
          <div style={{display:'flex',gap:8,flexWrap:'wrap'}}>
            {ALL_BADGES.filter(b => claimedSet.has(b.id)).map(badge => (
              <div key={badge.id} style={{
                padding:'4px 12px',borderRadius:20,fontSize:11,fontWeight:700,
                background:badge.color,color:'#fff',
                boxShadow:`0 2px 6px ${badge.color}66`,
              }}>{badge.name}</div>
            ))}
          </div>
          <button onClick={()=>{setShowBadgeBook(true);setBadgePage(0)}} style={{marginTop:8,fontSize:11,color:'var(--color-brand)',background:'none',border:'none',cursor:'pointer',padding:0}}>バッジ図鑑を見る →</button>
        </div>
      )}
      ...
      {followingAuthors.length > 0 && (
        <div style={{marginBottom:24}}>
          <div style={{fontSize:13,fontWeight:700,color:'var(--color-text)',marginBottom:10}}>フォロー中の作者（{followingAuthors.length}）</div>
      ...
      <div style={{display:'grid',gridTemplateColumns:'repeat(3,1fr)',gap:12}}>
        {[
          {label:'いいね', value:missionStats.likeCount, color:'var(--color-brand)'},
          {label:'コメント', value:missionStats.commentCount, color:'#3b82f6'},
          {label:'拡散', value:missionStats.discoverCount, color:'#22c55e'},
          {label:'保存', value:missionStats.bookmarkCount, color:'#8b5cf6'},
          {label:'フォロー', value:missionStats.followCount, color:'#f59e0b'},
          {label:'投稿話数', value:missionStats.episodeCount, color:'#ec4899'},
        ].map(item => (
          <div key={item.label} style={{padding:'12px 14px',background:'var(--color-bg)',borderRadius:10,textAlign:'center'}}>
        ...
        <div key={novel.id} style={{borderBottom:i<myNovels.length-1?'1px solid var(--color-brand-border)':'none',paddingBottom:16,marginBottom:16}}>
          ...
          <div style={{display:'flex',alignItems:'flex-start',gap:12,marginBottom:10,cursor:'pointer'}} onClick={()=>router.push(`/novel/${novel.id}`)}>
            <div style={{flex:1,minWidth:0}}>
              ...
              <div style={{fontSize:15,fontWeight:700,color:'var(--color-text)',marginBottom:4}}>{novel.title}</div>
              {/* 統計バー */}
              <div style={{display:'flex',gap:14,flexWrap:'wrap',fontSize:12,color:'var(--color-text-muted)'}}>
                <span>♡ {fmtNum(novelLikeMap[novel.id]||0)}</span>
                <span>💬 {fmtNum(novelCommentMap[novel.id]||0)}</span>
                <span>👁 {fmtNum(novelViewMap[novel.id]||0)}</span>
                <span>{novelEpCountMap[novel.id]||0}話</span>
              </div>
            </div>
          </div>
          {/* 操作ボタン */}
      ...
      {bookmarkedNovels.length === 0 ? (
        ...
        return (
          <div key={bm.novel_id} style={{borderBottom:'1px solid var(--color-brand-border)',paddingBottom:14,marginBottom:14,cursor:'pointer'}} onClick={()=>router.push(`/novel/${n.id}`)}>
            ...
            <div style={{fontSize:15,fontWeight:700,color:'var(--color-text)',marginBottom:2}}>{n.title}</div>
            <div style={{fontSize:12,color:'var(--color-text-muted)',marginBottom:5}}>作者：{authorName}</div>
            {n.summary && <div style={{fontSize:12,color:'#5a3a20',lineHeight:1.7,marginBottom:5,overflow:'hidden',display:'-webkit-box',WebkitLineClamp:2,WebkitBoxOrient:'vertical' as any}}>{n.summary}</div>}
            {(n.tags||[]).length > 0 && (
              <div style={{display:'flex',gap:4,flexWrap:'wrap'}}>
                {(n.tags as string[]).slice(0,5).map((t:string) => (
  ...
  const HistoryTab = () => (
    <div>
      ...
      {historyItems.length === 0 ? (
        ...
        <div key={item.novelId} style={{borderBottom:'1px solid var(--color-brand-border)',paddingBottom:14,marginBottom:14}}>
          <div style={{display:'flex',gap:5,marginBottom:5,flexWrap:'wrap',alignItems:'center'}}>
          ...
          {item.summary && <div style={{fontSize:12,color:'#5a3a20',lineHeight:1.7,marginBottom:6,overflow:'hidden',display:'-webkit-box',WebkitLineClamp:2,WebkitBoxOrient:'vertical' as any}}>{item.summary}</div>}
          {item.tags.length > 0 && (
            ...
            <div style={{display:'flex',gap:6}}>
              {firstEpMap[item.novelId] && firstEpMap[item.novelId]!==item.epId && (
                <Link href={`/novel/${item.novelId}/episode/${firstEpMap[item.novelId]}`}
                  style={{display:'inline-block',padding:'5px 12px',background:'var(--color-brand)',color:'#fff',borderRadius:12,fontSize:11,fontWeight:600,textDecoration:'none',whiteSpace:'nowrap'}}>
                  最初から読む
                </Link>
              )}
  ...
  return (
    <div style={{minHeight:'100vh',background:'var(--color-bg)',fontFamily:"'Noto Sans JP',sans-serif"}}>
                  ...
                  {ALL_BADGES.slice(badgePage*perPage,(badgePage+1)*perPage).map(badge => {
                    ...
                    return (
                      <div key={badge.id} style={{display:'flex',flexDirection:'column',alignItems:'center',gap:6,opacity:isSlot?0.2:1}}>
                        <div style={{width:sz+10,height:sz+10,borderRadius:'50%',background:owned?`radial-gradient(circle at 40% 35%, rgba(255,255,255,0.9), ${badge.color} 40%, ${badge.color}bb)`:'radial-gradient(circle at 40% 35%, #aaa 0%, #666 60%, #444)',border:owned?`3px solid ${badge.color}`:'3px solid #888',display:'flex',alignItems:'center',justifyContent:'center',boxShadow:owned?`0 4px 14px ${badge.color}88`:'0 3px 8px rgba(0,0,0,0.25)',position:'relative',overflow:'hidden'}}>
                          {owned?<span style={{fontSize:isMobile?9:11,fontWeight:700,color:'#fff',textAlign:'center',lineHeight:1.25,padding:'0 4px',zIndex:1,textShadow:'0 1px 3px rgba(0,0,0,0.6)'}}>{badge.name.replace(' Lv.','\nLv.')}</span>
        ...
        <div style={{position:'fixed',inset:0,background:'rgba(0,0,0,0.5)',zIndex:1000,display:'flex',alignItems:'center',justifyContent:'center',padding:24}}>
          <div style={{background:'var(--color-bg-card)',borderRadius:16,padding:'28px',maxWidth:420,width:'100%',boxShadow:'0 20px 60px rgba(0,0,0,0.2)'}}>
            ...
            <div style={{display:'flex',gap:10}}>
              ...
              <button onClick={handleEmailChange} disabled={emailSaving} style={{flex:1,padding:'10px',border:'none',borderRadius:8,background:'var(--color-brand)',color:'#fff',fontSize:13,fontWeight:700,cursor:'pointer',opacity:emailSaving?0.6:1}}>{emailSaving?'送信中...':'変更する'}</button>
            </div>
          </div>
        </div>
      )}
        ...
        <div style={{position:'fixed',inset:0,background:'rgba(0,0,0,0.5)',zIndex:1000,display:'flex',alignItems:'center',justifyContent:'center',padding:24}}>
          <div style={{background:'var(--color-bg-card)',borderRadius:16,padding:'28px',maxWidth:420,width:'100%',boxShadow:'0 20px 60px rgba(0,0,0,0.2)'}}>
            ...
            <div style={{display:'flex',gap:10}}>
              ...
              <button onClick={handlePwChange} disabled={pwSaving} style={{flex:1,padding:'10px',border:'none',borderRadius:8,background:'var(--color-brand)',color:'#fff',fontSize:13,fontWeight:700,cursor:'pointer',opacity:pwSaving?0.6:1}}>{pwSaving?'変更中...':'変更する'}</button>
            </div>
          </div>
        </div>
      )}
        ...
        <div style={{position:'fixed',inset:0,background:'rgba(0,0,0,0.5)',zIndex:1000,display:'flex',alignItems:'center',justifyContent:'center',padding:24}}>
          <div style={{background:'var(--color-bg-card)',borderRadius:16,padding:'28px',maxWidth:380,width:'100%',boxShadow:'0 20px 60px rgba(0,0,0,0.2)'}}>
            ...
            <div style={{display:'flex',gap:10}}>
              ...
              <button onClick={handleSaveBirthdate} disabled={bdSaving} style={{flex:1,padding:'10px',border:'none',borderRadius:8,background:'var(--color-brand)',color:'#fff',fontSize:13,fontWeight:700,cursor:'pointer',opacity:bdSaving?0.6:1}}>{bdSaving?'保存中…':'設定する'}</button>
            </div>
          </div>
        </div>
      )}
        ...
        <div style={{position:'fixed',inset:0,background:'rgba(0,0,0,0.5)',zIndex:1000,display:'flex',alignItems:'center',justifyContent:'center',padding:24}}>
          <div style={{background:'var(--color-bg-card)',borderRadius:16,padding:'28px',maxWidth:480,width:'100%',boxShadow:'0 20px 60px rgba(0,0,0,0.2)'}}>
            ...
            <div style={{display:'flex',gap:10}}>
              ...
              <button onClick={handleSaveBio} disabled={bioSaving} style={{flex:1,padding:'10px',border:'none',borderRadius:8,background:'var(--color-brand)',color:'#fff',fontSize:13,fontWeight:700,cursor:'pointer',opacity:bioSaving?0.6:1}}>{bioSaving?'保存中...':'保存する'}</button>
            </div>
          </div>
        </div>
      )}
        ...
        <div style={{position:'fixed',inset:0,background:'rgba(0,0,0,0.5)',zIndex:1000,display:'flex',alignItems:'center',justifyContent:'center',padding:24}}>
          <div style={{background:'var(--color-bg-card)',borderRadius:16,padding:'32px',maxWidth:480,width:'100%',boxShadow:'0 20px 60px rgba(0,0,0,0.2)'}}>
            ...
            <div style={{display:'flex',gap:10,marginTop:8}}>
              ...
              <button onClick={handleWithdraw} disabled={!withdrawPw||withdrawing} style={{flex:1,padding:'10px',border:'none',borderRadius:8,background:withdrawPw?'var(--color-danger)':'#f5f5f5',color:withdrawPw?'#fff':'var(--color-text-faint)',fontSize:13,fontWeight:700,cursor:withdrawPw?'pointer':'not-allowed'}}>{withdrawing?'処理中...':'退会する'}</button>
            </div>
          </div>
        </div>
      )}
          ...
          <div style={{background:'var(--color-bg-card)',borderRadius:16,padding:'28px',maxWidth:460,width:'100%',boxShadow:'0 20px 60px rgba(0,0,0,0.2)'}}>
            ...
            {deleteMode==='episode'&&deleteEpId&&(<>
              ...
              <div style={{display:'flex',gap:10}}>
                ...
                <button onClick={handleDeleteConfirm} disabled={deleteLoading} style={{flex:1,padding:'10px',border:'none',borderRadius:8,background:'var(--color-danger)',color:'#fff',fontSize:13,fontWeight:700,cursor:'pointer',opacity:deleteLoading?0.6:1}}>{deleteLoading?'削除中…':'削除する'}</button>
              </div>
            </>)}
            {deleteMode==='novel'&&(<>
              <div style={{background:'#fef2f2',border:'1px solid #fca5a5',borderRadius:8,padding:'12px 14px',marginBottom:16,fontSize:13,color:'var(--color-danger)',lineHeight:1.7}}>
              ...
              <div style={{display:'flex',gap:10}}>
                ...
                <button onClick={handleDeleteConfirm} disabled={deleteLoading} style={{flex:1,padding:'10px',border:'none',borderRadius:8,background:'var(--color-danger)',color:'#fff',fontSize:13,fontWeight:700,cursor:'pointer',opacity:deleteLoading?0.6:1}}>{deleteLoading?'削除中…':'完全に削除する'}</button>
              </div>
            </>)}
          </div>
        </div>
      ...
      {toast && (
        <div style={{position:'fixed',bottom:isMobile?80:24,right:24,background:'var(--color-brand)',color:'#fff',padding:'12px 20px',borderRadius:12,fontSize:13,fontWeight:600,zIndex:999,boxShadow:'0 4px 16px rgba(242,106,33,.35)'}}>
          {toast}
        </div>
      )}
