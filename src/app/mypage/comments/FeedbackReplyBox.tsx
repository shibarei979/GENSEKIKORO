        ...
        <div>
          ...
          <div style={{ display: 'flex', gap: 8, justifyContent: 'flex-end', marginTop: 6 }}>
            ...
            <button onClick={handleSubmit} disabled={posting || !body.trim()}
              style={{ fontSize: 12, fontWeight: 700, color: '#fff', background: 'var(--color-brand)', border: 'none', borderRadius: 8, padding: '6px 16px', cursor: posting || !body.trim() ? 'not-allowed' : 'pointer', opacity: posting || !body.trim() ? 0.5 : 1 }}>
              {posting ? '送信中...' : '返信する'}
            </button>
          </div>
        </div>
