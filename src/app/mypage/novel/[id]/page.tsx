...
import NovelManageActions from './NovelManageActions'
      ...
      <div style={{ maxWidth: 820, margin: '0 auto', padding: '24px 16px' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: 10, marginBottom: 18 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 8, flexWrap: 'wrap' }}>
            <span style={{ fontSize: 11, fontWeight: 700, color: '#fff', background: novel.is_serial ? 'var(--color-info)' : 'var(--color-text-faint)', padding: '3px 10px', borderRadius: 4 }}>{novel.is_serial ? '連載中' : '完結'}</span>
        ...
        <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap', marginBottom: 18 }}>
          <Link href={`/post?novel=${novel.id}`} style={{ background: 'var(--color-brand)', color: '#fff', fontSize: 12.5, fontWeight: 700, padding: '9px 18px', borderRadius: 16, textDecoration: 'none' }}>＋ 新しい話を追加</Link>
          ...
          {episodes.length === 0 ? (
            ...
            episodes.map((ep: any, i: number) => (
              <div key={ep.id} style={{ display: 'flex', alignItems: 'center', padding: '11px 18px', borderBottom: i === episodes.length - 1 ? 'none' : '1px solid var(--color-brand-light)', gap: 10 }}>
                <span style={{ fontSize: 10, color: '#fff', background: ep.published === false ? 'var(--color-text-faint)' : 'var(--color-info)', padding: '2px 7px', borderRadius: 3, flexShrink: 0 }}>{ep.published === false ? '下書き' : '公開'}</span>
