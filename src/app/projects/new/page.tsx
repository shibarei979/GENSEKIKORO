      ...
      <div style={{ maxWidth: 600, margin: '0 auto', padding: '24px 16px' }}>
        ...
        <div style={{ background: 'var(--color-bg-card)', border: '1px solid var(--color-brand-border)', borderRadius: 12, padding: '20px', display: 'flex', flexDirection: 'column', gap: 18 }}>
          ...
          <button onClick={handleSubmit} disabled={saving || !title.trim()}
            style={{ background: 'var(--color-brand)', color: 'var(--color-text-inverse)', border: 'none', borderRadius: 10, padding: '12px', fontSize: 14, fontWeight: 700, cursor: saving || !title.trim() ? 'not-allowed' : 'pointer', opacity: saving || !title.trim() ? 0.5 : 1 }}>
            {saving ? '作成中...' : '企画を作成する'}
          </button>
        </div>
      </div>
