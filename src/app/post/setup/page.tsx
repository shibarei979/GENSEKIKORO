        ...
        {step === 'existing' && (
          <>
            ...
            <button onClick={handleExistingGo}
              style={{width:'100%',marginTop:20,padding:'13px',background:'var(--color-brand)',color:'var(--color-text-inverse)',border:'none',borderRadius:10,fontSize:14,fontWeight:700,cursor:'pointer'}}>
              この作品に追加する
            </button>
          </>
        )}
        ...
        {step === 'new' && (
          <>
            ...
            <div style={fg}>
              <div style={{display:'flex',alignItems:'center',gap:6,marginBottom:5}}>
                ...
                <button type="button" onClick={()=>setShowCatchcopyHint(!showCatchcopyHint)}
                  style={{width:18,height:18,borderRadius:'50%',border:'1.5px solid var(--color-brand-border)',background:showCatchcopyHint?'var(--color-brand)':'var(--color-bg-card)',color:showCatchcopyHint?'var(--base-color-1)':'var(--color-text-muted)',fontSize:11,fontWeight:700,cursor:'pointer',display:'flex',alignItems:'center',justifyContent:'center',flexShrink:0,padding:0}}>
                  ？
                </button>
              </div>
                ...
                {GENRES.map(g=>{
                  ...
                  return (
                    <button key={g} type="button"
                      ...
                      style={{position:'relative',padding:'5px 14px',borderRadius:16,fontSize:12,border:`1.5px solid ${on?'var(--color-brand)':'var(--color-brand-border)'}`,cursor:'pointer',background:on?'var(--color-brand)':'var(--color-bg-card)',color:on?'var(--base-color-1)':'var(--color-text-muted)'}}>
                      {g}{idx===0 && <span style={{marginLeft:5,fontSize:9,background:'var(--color-bg-card)',color:'var(--color-brand)',padding:'1px 5px',borderRadius:8,fontWeight:700}}>主</span>}
                    </button>
                  )
                })}
              </div>
            ...
            <div style={fg}>
              ...
              <div style={{display:'flex',gap:6}}>
                ...
                <button onClick={()=>setShowTagExamples(!showTagExamples)} type="button"
                  style={{padding:'8px 12px',border:'1px solid var(--color-brand-border)',borderRadius:8,fontSize:14,fontWeight:700,color:showTagExamples?'var(--base-color-1)':'var(--color-brand)',background:showTagExamples?'var(--color-brand)':'var(--color-bg-card)',cursor:'pointer'}}>
                  ＋
                </button>
              </div>
              {showTagExamples && (
              ...
              <div style={{display:'flex',gap:8,flexWrap:'wrap'}}>
                {[
                  ...
                  <button key={btn.label} type="button" onClick={btn.onClick}
                    style={{padding:'5px 16px',borderRadius:16,border:`1.5px solid ${btn.active?btn.color:'var(--color-brand-border)'}`,fontSize:12,fontWeight:600,cursor:'pointer',background:btn.active?btn.color:'var(--color-bg-card)',color:btn.active?'var(--base-color-1)':btn.color}}>
                    {btn.label}
                  </button>
                ))}
              </div>
            ...
            <button onClick={handleCreateAndGo} disabled={loading}
              style={{width:'100%',padding:'14px',background:'var(--color-brand)',color:'var(--color-text-inverse)',border:'none',borderRadius:10,fontSize:15,fontWeight:700,cursor:'pointer',opacity:loading?0.6:1}}>
