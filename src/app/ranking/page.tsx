...
export default async function RankingPage({ searchParams }: Props) {
  ...
  async function getRanking(): Promise<{ items: any[]; total: number }> {
    ...
    if (period === 'newbie') {
      ...
      const newAuthorIds = [...new Set((newAuthors||[]).map((n: any) => n.author_id))]
      ...
      const aIds2 = [...new Set(paginated2.map((n: any) => n.author_id))]
    ...
    const authorIds = [...new Set(paged.map((n: any) => n.author_id))]
