...
async function computeRanking(period: string, novelType: string, serial: string, genre: string, aiMode: string, offset: number, displaySize: number, showMore: boolean): Promise<{ items: any[]; total: number }> {
  ...
  const supabase: any = createSbClient(serverEnv.NEXT_PUBLIC_SUPABASE_URL, serverEnv.NEXT_PUBLIC_SUPABASE_ANON_KEY)
  ...
  if (offset === 0 && genre === '全て' && novelType === '全て' && serial === 'all' && aiMode === 'human' && serverEnv.SUPABASE_SERVICE_ROLE_KEY) {
    try {
      const admin: any = createSbClient(serverEnv.NEXT_PUBLIC_SUPABASE_URL, serverEnv.SUPABASE_SERVICE_ROLE_KEY)
      const now = new Date()
