...
export default function HomeSelectPage() {
  ...
  return (
    <div style={{
      ...
      {/* 背景装飾 */}
      <div className="bg-blob" style={{ width: 460, height: 460, background: 'color-mix(in srgb, var(--color-brand) 10%, transparent)', top: '-140px', left: '-120px' }}/>
      ...
      <div className="bg-dot" style={{ width: 10, height: 10, background: 'color-mix(in srgb, var(--color-brand) 28%, transparent)', top: '16%', left: '18%', animation: 'floaty 6s ease-in-out infinite' }}/>
      ...
      <div className="bg-dot" style={{ width: 8, height: 8, background: 'color-mix(in srgb, var(--color-brand) 22%, transparent)', bottom: '20%', right: '26%', animation: 'floaty 6.5s ease-in-out infinite 1.2s' }}/>

      {step === 'role' && (
        <>
          {/* 左右のイラスト */}
          <img src="/writer.webp" alt="" className="role-illust role-illust-left"/>
          <img src="/reader.webp" alt="" className="role-illust role-illust-right"/>

          {/* ロゴ */}
          <img src="/logo.png" alt="原石航路"
            style={{ height: 'clamp(74px, 9vw, 108px)', objectFit: 'contain', marginBottom: 22, zIndex: 1, filter: 'drop-shadow(0 3px 10px color-mix(in srgb, var(--color-brand) 16%, transparent))' }}/>
          ...
          <div style={{ display: 'flex', gap: 28, alignItems: 'stretch', zIndex: 1, flexWrap: 'wrap', justifyContent: 'center' }}>
            ...
            <button onClick={() => handleSelectRole('writer')} disabled={loading} className="role-card"
              style={{ width: 260, background: 'var(--color-bg-card)', border: '1px solid #F0E4D8', borderRadius: 18, overflow: 'hidden', cursor: 'pointer', padding: 0, display: 'flex', flexDirection: 'column' }}>
              ...
              <div style={{ background: 'var(--color-brand)', color: 'var(--color-text-inverse)', padding: '18px 20px', fontSize: 15, fontWeight: 700, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 10 }}>
                書く人ではじめる
            ...
            <button onClick={() => handleSelectRole('reader')} disabled={loading} className="role-card"
              style={{ width: 260, background: 'var(--color-bg-card)', border: '1px solid #DDEEEB', borderRadius: 18, overflow: 'hidden', cursor: 'pointer', padding: 0, display: 'flex', flexDirection: 'column' }}>
              <div style={{ padding: '36px 24px 28px', flex: 1 }}>
              ...
              <div style={{ background: TEAL, color: 'var(--color-text-inverse)', padding: '18px 20px', fontSize: 15, fontWeight: 700, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 10 }}>
                読む人ではじめる
                <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"><polyline points="9 18 15 12 9 6"/></svg>
              </div>
            </button>
      ...
      {step === 'ai' && role === 'reader' && (
        <>
          <img src="/logo.png" alt="原石航路"
            style={{ height: 'clamp(62px, 7vw, 86px)', objectFit: 'contain', marginBottom: 20, zIndex: 1, filter: 'drop-shadow(0 3px 10px color-mix(in srgb, var(--color-brand) 16%, transparent))' }}/>
          ...
          <div style={{ display: 'flex', gap: 24, alignItems: 'stretch', flexWrap: 'wrap', justifyContent: 'center' }}>
            <button onClick={() => handleReaderAi(true)} disabled={loading} className="role-card"
              style={{ width: 250, background: 'var(--color-bg-card)', border: '1px solid #F0E4D8', borderRadius: 18, overflow: 'hidden', cursor: 'pointer', padding: 0, display: 'flex', flexDirection: 'column' }}>
              ...
              <div style={{ background: 'var(--color-brand)', color: 'var(--color-text-inverse)', padding: '16px 20px', fontSize: 14.5, fontWeight: 700, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 10 }}>
                これではじめる
            ...
            <button onClick={() => handleReaderAi(false)} disabled={loading} className="role-card"
              style={{ width: 250, background: 'var(--color-bg-card)', border: '1px solid #DDEEEB', borderRadius: 18, overflow: 'hidden', cursor: 'pointer', padding: 0, display: 'flex', flexDirection: 'column' }}>
              <div style={{ padding: '32px 22px 24px', flex: 1 }}>
              ...
              <div style={{ background: TEAL, color: 'var(--color-text-inverse)', padding: '16px 20px', fontSize: 14.5, fontWeight: 700, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 10 }}>
                これではじめる
                <svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"><polyline points="9 18 15 12 9 6"/></svg>
              </div>
            </button>
          ...
          <div style={{ display: 'flex', gap: 20, alignItems: 'stretch', flexWrap: 'wrap', justifyContent: 'center' }}>
            {([
              ...
              <button key={opt.v} onClick={() => handleWriterAi(opt.v as any)} disabled={loading} className="role-card"
                style={{ width: 230, background: 'var(--color-bg-card)', border: `1px solid ${opt.border}`, borderRadius: 18, overflow: 'hidden', cursor: 'pointer', padding: 0, display: 'flex', flexDirection: 'column' }}>
                ...
                <div style={{ background: opt.color, color: 'var(--color-text-inverse)', padding: '15px 18px', fontSize: 14, fontWeight: 700, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8 }}>
                  これではじめる
